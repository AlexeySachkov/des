...
#define GET_BIT(block, pos) ( ( ( block ) & ( 1 << ( pos ) ) ) != 0 )
#define UPD_BIT(block, bit, pos) ( ( ( bit ) == 0 ) ? CLR_BIT( block, pos ) : SET_BIT( block, pos ) )
#define CLR_BIT(block, pos) ( ( block ) &= ( ~ ( 1 << ( pos ) ) ) )
#define SET_BIT(block, pos) ( ( block ) |= ( 1 << ( pos ) ) )
...
block_t permutate(block_t block, const permutation_table_t permutation_table)
{
	...
	{
		UPD_BIT(result, GET_BIT(block, permutation_table[i]));
	}
...
half_block_t f(half_block_t block, key_t key)
{
...
key_t shift_key_left(key_t key, int shift)
{
...
key_t shift_key_right(key_t key, int shift)
{
	for (int i = 0; i < shift; ++i)
	{
...
keys_t get_keys(key_t key)
{
	key_t t = permutate(key, K);
	keys_t result(16);
	for (int i = 0; i < 16; ++i)
	{
...
block_t transform(block_t block, key_t key)
{
...
block_t i_transform(block_t block, key_t key)
{
	half_block_t lo = LO(block);
	half_block_t hi = HI(block);

	return MAKE_BLOCK(lo ^ f(hi, key), lo);
}

data encrypt(const block_t data, key_t key)
{
	keys_t keys = get_keys(key);
	data = permutate(data, IP);
	for (int i = 0; i < 16; ++i)
	{
		data = transform(data, key[i]);
	}
	data = permutate(data, IPI);
	return data;
}

data decrypt(const block_t encrypted_data, key_t key)
{
